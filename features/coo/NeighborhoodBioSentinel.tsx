      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none text-rose-500">
            <Translate target={language}>Bio_Sentinel</Translate>
          </h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
            <Translate target={language}>Regional Outbreak Intercept</Translate>
          </p>
        </div>
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 backdrop-blur-xl">
           {['Toronto', 'Hamilton', 'Ottawa'].map(r => (
             <button key={r} onClick={() => setActiveRegion(r)} className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase transition-all ${activeRegion === r ? 'bg-rose-600 text-white shadow-2xl' : 'text-slate-500 hover:text-white'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
           ...
           {loading ? (
             <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                ...
                <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest animate-pulse">
                  <Translate target={language}>Ingesting_Public_Health_Telemetry</Translate>
                </p>
             </div>
           ) : (
                ...
                {zones.map((zone, i) => (
                  <div key={i} className={`p-8 rounded-[3rem] border transition-all ${zone.severity === 'CRITICAL' ? 'bg-rose-600/10 border-rose-500/50 shadow-[0_0_50px_rgba(244,63,94,0.1)]' : 'bg-white/[0.03] border-white/5'}`}>
                    ...
                    <p className="text-[10px] font-bold text-slate-500 uppercase mb-6">
                      <Translate target={language}>{zone.threatType}</Translate> <Translate target={language}>Threat</Translate> • <Translate target={language}>{zone.severity}</Translate>
                    </p>
                    ...
                    <div className="flex flex-wrap gap-2">
                       {zone.mandatoryPPE.map(item => (
                         <span key={item} className="px-3 py-1 bg-rose-600/20 text-rose-400 border border-rose-500/20 rounded text-[8px] font-black uppercase">
                            <Translate target={language}>{item}</Translate>
                         </span>
                       ))}
                    </div>
                  </div>
        ...
        <div className="lg:col-span-4 bg-rose-600 p-10 rounded-[3rem] text-white shadow-2xl flex flex-col group">
           <h3 className="text-xs font-black uppercase tracking-widest mb-10 opacity-60 italic">
             <Translate target={language}>Enforcement_Protocol</Translate>
           </h3>
           <p className="text-sm font-bold italic leading-relaxed mb-10">
             "<Translate target={language}>Neural Intercept enabled. Staff entering Critical zones must verify PPE via Vision Scan before authorization.</Translate>"
           </p>
           <button onClick={() => alert("PPE enforcement updated.")} className="w-full py-5 bg-white text-rose-600 rounded-2xl font-black text-[10px] uppercase shadow-xl hover:scale-105 active:scale-95 transition-all">
             <Translate target={language}>AUTHORIZE_LOCKDOWN</Translate>
           </button>
        </div>
      </div>
    </div>
