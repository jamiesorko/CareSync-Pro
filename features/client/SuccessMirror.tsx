...
const SuccessMirror: React.FC<Props> = ({ client, language }) => {
  ...
  return (
    <div className="bg-[#fef3c7] border border-amber-200 rounded-[4rem] p-12 shadow-2xl relative overflow-hidden group min-h-[500px] flex flex-col">
      <div className="absolute top-0 right-0 p-10 opacity-5 font-black italic text-9xl text-amber-900 pointer-events-none">
         <Translate target={language}>WIN</Translate>
      </div>
      
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center space-y-8">
           <div className="w-16 h-16 border-4 border-amber-900/10 border-t-amber-900 rounded-full animate-spin"></div>
           <p className="text-xl font-black text-amber-900 uppercase tracking-widest animate-pulse">
              <Translate target={language}>Finding_Your_Successes</Translate>
           </p>
        </div>
      ) : synthesis && (
        <div className="space-y-12 animate-in slide-in-from-bottom-8 duration-1000">
           <div className="space-y-4">
              <p className="text-xl font-black text-amber-800/60 uppercase tracking-widest leading-none">
                 <Translate target={language}>Todays_Progress</Translate>
              </p>
              <h2 className="text-6xl font-black text-amber-950 tracking-tighter italic leading-none uppercase">
                 <Translate target={language}>{synthesis.headline}</Translate>
              </h2>
           </div>
           ...
           <div className="space-y-6">
              {synthesis.accomplishments.map((win, i) => (
                <div key={i} className="flex items-center gap-8 bg-white/40 p-6 rounded-[2.5rem] border border-white group/win hover:scale-[1.02] transition-all">
                   ...
                   <p className="text-2xl font-bold text-amber-950 italic">
                      "<Translate target={language}>{win}</Translate>"
                   </p>
                </div>
              ))}
           ...
           <div className="mt-auto pt-10 border-t border-amber-900/10 flex justify-between items-end">
              <div>
                 <p className="text-[10px] font-black text-amber-800/50 uppercase tracking-widest mb-1">
                    <Translate target={language}>Your_Visitor</Translate>
                 </p>
                 <p className="text-2xl font-black text-amber-900 uppercase italic">
                    <Translate target={language}>{synthesis.visitorToday}</Translate>
                 </p>
              </div>
              <button className="px-10 py-4 bg-amber-900 text-white rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl">
                 <Translate target={language}>See_More</Translate>
              </button>
           </div>
        </div>
      )}
