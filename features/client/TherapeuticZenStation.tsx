...
const TherapeuticZenStation: React.FC<Props> = ({ client, language }) => {
  ...
  return (
    <div className="bg-[#0f172a] border border-white/10 rounded-[5rem] p-12 shadow-2xl relative overflow-hidden flex flex-col min-h-[500px]">
      <div className="absolute top-0 right-0 p-10 opacity-5 font-black italic text-9xl text-white pointer-events-none">
        <Translate target={language}>ZEN</Translate>
      </div>
      ...
      <div className="relative z-10 flex flex-col h-full space-y-12">
         <div className="flex items-center space-x-6">
            ...
            <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
              <Translate target={language}>Therapeutic_Zen_Station</Translate>
            </h3>
         </div>
         ...
         {videoUrl ? (
           <div className="flex-1 animate-in zoom-in duration-1000 space-y-8">
              ...
              <button onClick={() => setVideoUrl(null)} className="w-full py-6 text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-white">
                <Translate target={language}>Exit_Meditation</Translate>
              </button>
           </div>
         ) : (
           <div className="flex-1 flex flex-col justify-center space-y-12">
              <div className="max-w-2xl">
                 <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">
                   <Translate target={language}>Neural Suggestion for Calm</Translate>
                 </p>
                 <p className="text-3xl font-bold text-white italic leading-tight">
                    {loading ? <Translate target={language}>Thinking</Translate> : `"${prompt?.prompt}"`}
                 </p>
              </div>
              ...
              <div className="flex gap-4">
                 ...
                 >
                   {loading ? <Translate target={language}>Synthesizing_Zen</Translate> : <Translate target={language}>INITIALIZE_ZEN_MOMENT</Translate>}
                 </button>
                 <button onClick={initZen} className="px-10 py-6 bg-white/5 border border-white/10 text-slate-500 rounded-3xl font-black text-[10px] uppercase tracking-widest">
                   <Translate target={language}>New_Idea</Translate>
                 </button>
              </div>
           </div>
         )}
