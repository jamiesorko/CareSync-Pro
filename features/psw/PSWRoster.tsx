...
const PSWRoster: React.FC<Props> = ({ clients, onStartVisit }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in slide-in-from-bottom-4 duration-500 pb-20">
      {clients.map((client) => (
        <div key={client.id} className="alaya-card p-10 rounded-[3.5rem] shadow-2xl group border-orange-500/10 hover:border-orange-500/40 transition-all flex flex-col bg-[#020617]">
          ...
          <div className="p-8 bg-white/[0.03] border border-white/5 rounded-[2.5rem] mb-8 flex-1">
             <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-4">Patient_Mobility_Vector</p>
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className={`p-4 rounded-2xl border flex items-center gap-4 ${client.mobilityStatus.dementia ? 'bg-amber-500/10 border-amber-500/30 text-amber-500' : 'bg-white/5 border-white/5 text-slate-500'}`}>
                   <Brain size={18} />
                   <span className="text-[10px] font-black uppercase tracking-tighter">Dementia: {client.mobilityStatus.dementia ? 'YES' : 'NO'}</span>
                </div>
                <div className={`p-4 rounded-2xl border flex items-center gap-4 ${client.mobilityStatus.isBedridden ? 'bg-rose-500/10 border-rose-500/30 text-rose-500' : 'bg-white/5 border-white/5 text-slate-500'}`}>
                   ...
                   <span className="text-[10px] font-black uppercase tracking-tighter">Bedridden: {client.mobilityStatus.isBedridden ? 'YES' : 'NO'}</span>
                </div>
                <div className="p-4 bg-white/5 border border-white/5 rounded-2xl flex items-center gap-4 text-slate-300">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-tighter">Lift: {client.mobilityStatus.liftType}</span>
                </div>
                <div className="p-4 bg-white/5 border border-white/5 rounded-2xl flex items-center gap-4 text-slate-300">
                   <UserCheck size={18} className="text-emerald-400" />
                   <span className="text-[10px] font-black uppercase tracking-tighter">Method: {client.mobilityStatus.transferMethod}</span>
                </div>
             </div>

             <div className="mt-6 flex flex-wrap gap-2">
                {client.mobilityStatus.useWheelchair && <span className="px-3 py-1 bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 rounded text-[8px] font-black uppercase">Wheelchair</span>}
                {client.mobilityStatus.useWalker && <span className="px-3 py-1 bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 rounded text-[8px] font-black uppercase">Walker</span>}
             </div>
          </div>
          ...
          <div className="flex justify-between items-center opacity-40 group-hover:opacity-100 transition-opacity">
             <div className="flex gap-2">
                {client.conditions.slice(0, 3).map(c => (
                  <span key={c} className="text-[8px] font-black text-slate-500 uppercase tracking-widest">{c}</span>
                ))}
             </div>
