    ...
    <div className="space-y-4 animate-in fade-in duration-700 pb-8">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mt-1">Neural Training</p>
        </div>
        {!module && !loading && (
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <div className="lg:col-span-8 bg-slate-900 border border-white/10 rounded-2xl p-6 shadow-2xl relative overflow-hidden flex flex-col min-h-[300px]">
           {loading ? (
             ...
             <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6 animate-in zoom-in">
                ...
                <div className="flex gap-3">
                   <button onClick={() => setModule(null)} className="px-4 py-1.5 bg-white text-black rounded font-black text-[9px] uppercase tracking-widest">Return</button>
             ...
             <div className="flex-1 flex flex-col relative z-10 space-y-6 animate-in slide-in-from-bottom-4">
                <h3 className="text-lg font-black text-white italic tracking-tighter uppercase">{module.title}</h3>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {module.questions[activeQuestion].a.map((opt: string, i: number) => (
                        <button 
                         ...
                         className={`p-3 rounded-lg border text-left text-[9px] font-bold uppercase transition-all ${
                           selectedOpt === i 
             ...
             <div className="flex-1 flex flex-col items-center justify-center opacity-20 italic">
                <p className="text-xs font-bold uppercase tracking-widest">Awaiting Forge Input</p>
             </div>
           )}
        </div>
        <div className="lg:col-span-4 space-y-4 flex flex-col">
           <div className="bg-[#fbbf24] p-6 rounded-2xl text-black shadow-2xl">
              <h3 className="text-[8px] font-black uppercase tracking-widest mb-2 opacity-60">Status</h3>
              <p className="text-3xl font-black italic tracking-tighter">98%</p>
              <p className="text-[9px] font-bold italic leading-tight mt-2 opacity-80">Personal Competency Index synchronized.</p>
           </div>
        </div>
      </div>
    </div>
