  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 h-full">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-black text-white tracking-tighter uppercase italic leading-none">Workforce_Nexus</h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">Neural Synergy & Resilience Mapping</p>
        </div>
        <button 
          onClick={runAnalysis}
          disabled={loading}
        ...
        >
          {loading ? 'CALIBRATING...' : 'SYNC_PAIRINGS'}
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 h-full">
        <div className="lg:col-span-12 bg-slate-950 border border-white/10 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden flex flex-col min-h-[500px]">
           ...
           <div className="flex-1 space-y-6 overflow-y-auto scrollbar-hide pr-2 relative z-10">
              {pairings.length > 0 ? pairings.map((p, i) => {
                ...
                <div className="h-full flex items-center justify-center opacity-30 italic uppercase text-xs tracking-widest">Awaiting Neural Link Calibration...</div>
              )}
           </div>
        </div>
      </div>
