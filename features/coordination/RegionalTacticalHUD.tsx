      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none text-rose-500">
            <Translate target={language}>Regional_Tactical_Map</Translate>
          </h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
            <Translate target={language}>Grounded Logistics Impedance & Fleet Safety Intercept</Translate>
          </p>
        </div>
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 backdrop-blur-xl">
           {['Toronto', 'Hamilton', 'Ottawa'].map(r => (
             <button 
              key={r}
              onClick={() => setRegion(r)}
              className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase transition-all ${region === r ? 'bg-rose-600 text-white shadow-2xl' : 'text-slate-500 hover:text-white'}`}
             >
               <Translate target={language}>{r}</Translate>
             </button>
           ))}
        </div>
      </div>
        ...
        <div className="lg:col-span-8 bg-slate-950 border border-white/10 rounded-[4rem] overflow-hidden shadow-2xl relative min-h-[600px] flex flex-col">
           ...
           <div className="relative z-10 p-12 flex-1 flex flex-col">
              <h3 className="text-xl font-black text-white italic tracking-tighter uppercase mb-10">
                <Translate target={language}>Sector_Visual_Intercept</Translate>
              </h3>
              ...
              {loading ? (
                <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                   ...
                   <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest animate-pulse">
                     <Translate target={language}>Intercepting_Grounded_Signals</Translate>
                   </p>
                </div>
              ) : (
                   ...
                   {threats.map((t, i) => (
                     <div key={i} className={`p-10 rounded-[3rem] border transition-all animate-in slide-in-from-left-8 duration-700 ${t.severity === 'CRITICAL' ? 'bg-rose-600/20 border-rose-500' : 'bg-slate-900 border-white/10'}`}>
                        <div className="flex justify-between items-start mb-8">
                           <div>
                              <span className={`px-3 py-1 rounded text-[8px] font-black uppercase ${t.severity === 'CRITICAL' ? 'bg-rose-600' : 'bg-amber-600'} text-white`}>
                                <Translate target={language}>{t.severity}</Translate>_ALERT
                              </span>
                              <h4 className="text-3xl font-black text-white italic tracking-tighter uppercase mt-4 leading-none">
                                <Translate target={language}>{t.type}</Translate> <Translate target={language}>Hazard</Translate>
                              </h4>
                           </div>
                           <div className="text-right">
                              <p className="text-5xl font-black italic text-white tracking-tighter">
                                <Translate target={language}>{String(t.impactedStaffCount)}</Translate>
                              </p>
                              <p className="text-[7px] font-bold text-slate-500 uppercase">
                                <Translate target={language}>Staff Nodes Impacted</Translate>
                              </p>
                           </div>
                        </div>
                        <p className="text-lg font-bold text-slate-300 italic mb-10 leading-relaxed">
                          "<Translate target={language}>{t.description}</Translate>"
                        </p>
                        <div className="p-8 bg-black/40 border border-white/10 rounded-3xl">
                           <p className="text-[9px] font-black text-rose-500 uppercase tracking-widest mb-4">
                             <Translate target={language}>Tactical_Directive</Translate>
                           </p>
                           <p className="text-sm font-black text-white uppercase italic tracking-tighter leading-relaxed">
                             "<Translate target={language}>{t.tacticalDirective}</Translate>"
                           </p>
                        </div>
                     </div>
                   ))}
        ...
        <div className="lg:col-span-4 space-y-6 flex flex-col">
           <div className="bg-rose-600 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
              ...
              <h3 className="text-xs font-black uppercase tracking-widest mb-10 opacity-60 italic">
                <Translate target={language}>Force_Protection_Alert</Translate>
              </h3>
              <div className="space-y-8 relative z-10 flex-1">
                 <p className="text-sm font-bold italic leading-relaxed">
                   <Translate target={language}>Neural Dispatch identifies significant transit impedance in Sector 1. Initiating autonomous visit buffering: +15m added to all arrival windows.</Translate>
                 </p>
                 <button 
                 ...
                 >
                   <Translate target={language}>AUTHORIZE_FLEET_BUFFER</Translate>
                 </button>
              </div>
           </div>
           ...
           <div className="bg-white/5 border border-white/10 p-10 rounded-[3rem] flex-1 backdrop-blur-3xl overflow-hidden flex flex-col shadow-3xl">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-10 italic">
                <Translate target={language}>Dispatch_Pulse_Log</Translate>
              </p>
              <div className="flex-1 space-y-6 overflow-y-auto scrollbar-hide pr-2">
                 {[
                   ...
                   <div key={i} className="flex justify-between items-center py-3 border-b border-white/5 last:border-0">
                      ...
                      <span className={`text-[11px] font-black uppercase ${stat.color}`}>
                 ...
                 <div className="mt-8 p-6 bg-white/[0.02] border border-white/5 rounded-2xl">
                    <p className="text-[8px] font-black text-slate-600 uppercase mb-4 tracking-widest">
                      <Translate target={language}>Logistician_Insight</Translate>
                    </p>
                    <p className="text-[10px] text-slate-300 italic leading-relaxed">
                      <Translate target={language}>Cross-referencing transit drift with staff fuel usage. Detected 8% higher burn in Sector 4. Adjusting mileage reimbursement node for Oct-Cycle.</Translate>
                    </p>
                 </div>
              </div>
           </div>
