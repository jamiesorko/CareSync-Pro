      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none text-amber-500">
             <Translate target={language}>Cognitive_Matrix</Translate>
          </h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
             <Translate target={language}>Linguistic_personality_drift_tracking</Translate>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
           ...
           {loading ? (
             <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                ...
                <p className="text-[10px] font-black text-amber-500 uppercase tracking-[0.6em] animate-pulse">
                   <Translate target={language}>Analyzing_Personality_Vectors</Translate>
                </p>
             </div>
           ) : pulse && (
             <div className="space-y-12 relative z-10 animate-in zoom-in">
                <div className="text-center py-10 bg-amber-500/5 border border-amber-500/20 rounded-[3rem]">
                   <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-2">
                      <Translate target={language}>Cognitive_Drift_Magnitude</Translate>
                   </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl">
                      <p className="text-[9px] font-black text-slate-500 uppercase mb-4 tracking-widest">
                         <Translate target={language}>Linguistic_Attrition</Translate>
                      </p>
                      <div className="flex flex-wrap gap-2">
                         {pulse.attrition.map((a, i) => (
                           <span key={i} className="px-3 py-1 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded text-[9px] font-black uppercase">
                              <Translate target={language}>{a}</Translate>
                           </span>
                         ))}
                      </div>
                   </div>
                   <div className="p-8 bg-amber-500/5 border border-amber-500/10 rounded-3xl">
                      <p className="text-[9px] font-black text-amber-500 uppercase mb-4 tracking-widest">
                         <Translate target={language}>Confusion_Window</Translate>
                      </p>
                      <p className="text-xl font-black text-white uppercase italic">
                         <Translate target={language}>{pulse.window}</Translate>
                      </p>
                   </div>
                </div>
             </div>
        ...
        <div className="lg:col-span-4 bg-amber-600 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
           <h3 className="text-xs font-black uppercase tracking-widest mb-10 opacity-60 italic">
              <Translate target={language}>Reminiscence_Forge</Translate>
           </h3>
           <p className="text-sm font-bold italic leading-relaxed mb-10">
              "<Translate target={language}>AI_Suggestion_Forge_memory_anchor_for_deescalation</Translate>"
           </p>
           <button onClick={() => alert("Forging cinematic memory anchor.")} className="w-full py-5 bg-white text-amber-600 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl">
              <Translate target={language}>INITIALIZE_VEO_FORGE</Translate>
           </button>
        </div>
      </div>
    </div>
