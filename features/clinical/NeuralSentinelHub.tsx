      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none text-rose-500">
             <Translate target={language}>Neural_Sentinel</Translate>
          </h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
             <Translate target={language}>Chrono_Trajectory_Mapping</Translate>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 bg-black border border-white/10 rounded-[4rem] p-12 shadow-2xl relative overflow-hidden flex flex-col min-h-[600px]">
           ...
           {loading ? (
             <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                ...
                <p className="text-[10px] font-black text-rose-500 uppercase tracking-[0.6em] animate-pulse">
                   <Translate target={language}>Running_Trajectory_Simulation</Translate>
                </p>
             </div>
           ) : activeSim && (
             <div className="space-y-12 relative z-10 animate-in zoom-in">
                <div className="p-8 bg-rose-600/5 border border-rose-500/20 rounded-3xl">
                   <p className="text-[9px] font-black text-rose-500 uppercase mb-4">
                      <Translate target={language}>Crisis_Probability_Horizon</Translate>
                   </p>
                ...
                <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl">
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-6">
                      <Translate target={language}>Simulation_Reasoning_Chain</Translate>
                   </p>
                      ...
                      {activeSim.reasoningChain.map((step, i) => (
                        <div key={i} className="flex items-center gap-4">
                           ...
                           <p className="text-sm text-slate-300 italic">
                              "<Translate target={language}>{step}</Translate>"
                           </p>
                        </div>
                      ))}
                   </div>
        ...
        <div className="lg:col-span-4 bg-rose-600 p-10 rounded-[3rem] text-white shadow-2xl">
           <h3 className="text-xs font-black uppercase tracking-widest mb-10 opacity-60">
              <Translate target={language}>Preventative_Directive</Translate>
           </h3>
           <p className="text-lg font-bold italic leading-tight mb-10 uppercase">
              "<Translate target={language}>{activeSim?.suggestedDirective || ""}</Translate>"
           </p>
           <button onClick={() => alert("Preventative order pushed.")} className="w-full py-5 bg-white text-rose-600 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl">
              <Translate target={language}>PUBLISH_INTERCEPT</Translate>
           </button>
        </div>
      </div>
    </div>
