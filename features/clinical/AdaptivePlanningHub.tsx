  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-1000 pb-24">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none text-purple-400">
            <Translate target={language}>Bio_Social_Core</Translate>
          </h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
            <Translate target={language}>Social_determinant_integration</Translate>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 bg-slate-950 border border-white/10 rounded-[4rem] p-12 shadow-2xl relative overflow-hidden flex flex-col min-h-[600px]">
           ...
           {loading ? (
             <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                ...
                <p className="text-[10px] font-black text-purple-500 uppercase tracking-[0.6em] animate-pulse">
                  <Translate target={language}>Correlating_Social_Vectors</Translate>
                </p>
             </div>
           ) : pulse && (
             <div className="space-y-12 relative z-10 animate-in slide-in-from-bottom-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl">
                      <p className="text-[9px] font-black text-slate-500 uppercase mb-6">
                        <Translate target={language}>Isolation</Translate>
                      </p>
                      <p className="text-5xl font-black italic tracking-tighter text-amber-500">{pulse.isolationScore}%</p>
                   </div>
                   <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl">
                      <p className="text-[9px] font-black text-slate-500 uppercase mb-6">
                        <Translate target={language}>Nutrition</Translate>
                      </p>
                      <p className="text-xl font-black italic uppercase text-rose-500">
                        <Translate target={language}>{pulse.nutritionDrift}</Translate>
                      </p>
                   </div>
                   <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl">
                      <p className="text-[9px] font-black text-slate-500 uppercase mb-6">
                        <Translate target={language}>Env_Integrity</Translate>
                      </p>
                ...
                <div className="p-10 bg-purple-600/5 border border-purple-500/20 rounded-[3rem]">
                   <p className="text-[9px] font-black text-purple-400 uppercase mb-6">
                     <Translate target={language}>AI_Synthesis</Translate>
                   </p>
                   <p className="text-sm text-slate-200 leading-relaxed font-medium italic italic">
                     "<Translate target={language}>{pulse.aiSynthesis}</Translate>"
                   </p>
                </div>
             </div>
           )}
        ...
        <div className="lg:col-span-4 bg-[#fbbf24] p-10 rounded-[3rem] text-black shadow-2xl relative overflow-hidden">
           <h3 className="text-xs font-black uppercase tracking-widest mb-10 opacity-60 italic">
             <Translate target={language}>Neighborhood_Immunity</Translate>
           </h3>
           {threat && (
             <div className="space-y-10 relative z-10 animate-in slide-in-from-right-4">
                <p className="text-3xl font-black italic uppercase leading-none">
                  <Translate target={language}>{threat.threatType}</Translate>
                </p>
                <div className="p-6 bg-black/5 border border-black/10 rounded-2xl text-xs font-bold italic italic">
                  "<Translate target={language}>{threat.mandateUpdate}</Translate>"
                </div>
                <button onClick={() => alert("PPE vector updated.")} className="w-full py-5 bg-black text-white rounded-2xl font-black text-[10px] uppercase shadow-xl">
                  <Translate target={language}>UPDATE_FLEET_PPE</Translate>
                </button>
             </div>
           )}
        </div>
