...
const AccountsPayable: React.FC<Props> = ({ language }) => {
  ...
  const verifyPayable = (id: string) => { setPayables(prev => prev.map(p => p.id === id ? { ...p, status: 'VERIFIED' } : p)); };
  const processPayment = (id: string) => { setPayables(prev => prev.map(p => p.id === id ? { ...p, status: 'PAID' } : p)); };
  const orderSupplies = (id: string) => { setSupplyRequests(prev => prev.map(r => r.id === id ? { ...r, status: 'ORDERED' } : r)); };
            ...
            <div key={p.id} className="p-6 bg-white/[0.03] border border-white/5 rounded-2xl flex justify-between items-center group">
              <div>
                ...
                <div className="mt-2 flex items-center space-x-2">
                  <span className={`text-[8px] font-black px-2 py-0.5 rounded ${p.status === 'PAID' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
