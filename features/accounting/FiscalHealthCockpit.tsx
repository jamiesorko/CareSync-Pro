    ...
    <div className="h-full space-y-12 animate-in fade-in duration-1000 pb-24">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none text-indigo-400">
             <Translate target={language}>FISCAL_HEALTH_COCKPIT</Translate>
          </h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
             <Translate target={language}>Autonomous_Capital_Forensics_&_Micro-Leakage_Intercept_Hub</Translate>
          </p>
        </div>
        <button 
          onClick={runDeepScan}
        ...
        >
          {loading ? <Translate target={language}>INTERROGATING_LEDGERS</Translate> : <Translate target={language}>INITIATE_DEEP_FISCAL_AUDIT</Translate>}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 bg-slate-950 border border-white/10 rounded-[4rem] p-10 shadow-2xl relative overflow-hidden flex flex-col min-h-[650px]">
           ...
           <h3 className="text-xl font-black text-white italic tracking-tighter uppercase mb-10 relative z-10">
              <Translate target={language}>NEURAL_LEAKAGE_INBOUND</Translate>
           </h3>
           ...
           <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10 overflow-y-auto scrollbar-hide pr-2">
              {loading ? (
                 <div className="col-span-2 flex flex-col items-center justify-center space-y-8 h-full">
                    ...
                    <p className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.6em] animate-pulse">
                       <Translate target={language}>Auditing_14,000_Visit_Nodes</Translate>
                    </p>
                 </div>
              ) : signals.map((s, i) => (
                <div 
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    <div className={`px-3 py-1 rounded text-[8px] font-black uppercase ${
                      ...
                      s.type === 'MILEAGE' ? 'bg-amber-600 text-white' : 'bg-rose-600 text-white'
                       ...
                       <Translate target={language}>{s.type}</Translate>
                    </div>
                  ...
                  <h4 className="text-sm font-black text-white uppercase italic tracking-tighter mb-2">
                     <Translate target={language}>Vector</Translate>: <Translate target={language}>{s.involvedStaff}</Translate>
                  </h4>
                  <p className="text-[10px] text-slate-500 font-medium italic leading-relaxed line-clamp-2">
                     "<Translate target={language}>{s.rationale}</Translate>"
                  </p>
                  
                  <div className="mt-6 flex justify-between items-baseline">
                     <div className="flex items-center space-x-2">
                        <span className="text-[8px] font-black text-slate-700 uppercase"><Translate target={language}>Certainty_Index</Translate></span>
        ...
        <div className="lg:col-span-4 space-y-6 flex flex-col">
           <div className="bg-[#1e1b4b] border border-white/10 p-10 rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col">
              ...
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 italic">
                 <Translate target={language}>Neural_Forensic_Interrogator</Translate>
              </h3>
              ...
              {selectedSignal ? (
                <div className="flex-1 flex flex-col space-y-10 animate-in slide-in-from-right-4 duration-500">
                   <div className="bg-white/5 p-8 rounded-3xl border border-white/10 relative overflow-hidden group">
                      ...
                      <p className="text-[8px] font-black text-indigo-400 uppercase mb-4 tracking-widest">
                         <Translate target={language}>Logic_Evidence_Interrogation</Translate>
                      </p>
                      <p className="text-sm font-bold text-white italic leading-relaxed">
                        <Translate target={language}>{selectedSignal.rationale}</Translate>
                      </p>
                   </div>

                   <div className="space-y-6">
                      <div className="flex justify-between items-center py-2 border-b border-white/5">
                         <span className="text-[9px] font-bold text-slate-500 uppercase"><Translate target={language}>Impact_Probability</Translate></span>
                         <span className="text-[10px] font-black text-rose-500 uppercase italic tracking-widest"><Translate target={language}>CRITICAL_EXPOSURE</Translate></span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-white/5">
                         <span className="text-[9px] font-bold text-slate-500 uppercase"><Translate target={language}>Reclamation_Potential</Translate></span>
                         <span className="text-[10px] font-black text-emerald-400 uppercase italic tracking-widest"><Translate target={language}>HIGH_ROI</Translate></span>
                      </div>
                   </div>
                   ...
                   >
                     <Translate target={language}>EXECUTE_RECLAIM</Translate>
                   </button>
                </div>
              ) : (
                <div className="flex-1 flex flex-col items-center justify-center opacity-20 italic text-center px-10 space-y-6">
                   ...
                   <p className="text-sm leading-relaxed">
                      <Translate target={language}>Select_a_capital_drift_vector_from_the_matrix_to_initialize_a_deep_forensic_Interrogation_cycle</Translate>
                   </p>
                </div>
              )}
           </div>
           ...
           <div className="bg-indigo-600 p-10 rounded-[3rem] text-white shadow-2xl shadow-indigo-600/30">
              <p className="text-[9px] font-black uppercase tracking-widest mb-4 opacity-60">
                 <Translate target={language}>CFO_OPERATIONAL_DIRECTIVE</Translate>
              </p>
              <div className="flex items-baseline space-x-2 mb-6">
                 <p className="text-6xl font-black italic tracking-tighter">
                    <Translate target={language}>1.8%</Translate>
                 </p>
                 <span className="text-xs font-black opacity-50 uppercase tracking-widest">
                    <Translate target={language}>Target_Leakage_Floor</Translate>
                 </span>
              </div>
              <p className="text-xs font-bold italic leading-relaxed">
                "<Translate target={language}>Autonomous_forensics_have_identified_a_recurring_pattern_of_Phantom_Mileage_in_Sector_1_Initiating_geo-boundary_refinement_to_100m_buffer</Translate>"
              </p>
           </div>
        </div>
