  ...
  return (
    <div className="h-full space-y-12 animate-in fade-in duration-1000 pb-24">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none text-emerald-500">
            <Translate target={language}>Revenue_Reclamation_Node</Translate>
          </h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">
            <Translate target={language}>Autonomous_Evidence_Harvesting_&_Forensic_Denial_Response</Translate>
          </p>
        </div>
        <button 
        ...
        >
          {loading ? <Translate target={language}>INGESTING_DENIAL_VECTORS</Translate> : <Translate target={language}>DISCOVER_RECLAIMABLE_ASSETS</Translate>}
        </button>
      </div>
        ...
        <div className="lg:col-span-5 space-y-6 flex flex-col">
           <div className="bg-slate-900 border border-white/10 p-10 rounded-[3rem] shadow-2xl flex flex-col flex-1 relative overflow-hidden">
              ...
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-10 italic">
                <Translate target={language}>Inbound_Fiscal_Gap_Queue</Translate>
              </h3>
              ...
              <div className="flex-1 space-y-4 overflow-y-auto scrollbar-hide pr-2">
                 {cases.map((c, i) => (
                   ...
                   >
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-400 text-[7px] font-black rounded uppercase">
                          <Translate target={language}>RECOVERABLE</Translate>
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-500 font-bold italic mb-4 line-clamp-2">
                        "<Translate target={language}>{c.denialReason}</Translate>"
                      </p>
                      <div className="flex justify-between items-baseline">
                         <span className="text-[9px] font-black text-emerald-400 uppercase">{c.successProbability}% <Translate target={language}>Success_Probability</Translate></span>
                 ...
                 {cases.length === 0 && !loading && (
                   <div className="h-full flex flex-col items-center justify-center opacity-20 italic">
                      <p className="text-sm">
                        <Translate target={language}>Awaiting_capital_discovery_signal</Translate>
                      </p>
                   </div>
                 )}
              </div>
           ...
           <div className="bg-emerald-600 p-10 rounded-[3rem] text-white shadow-2xl shadow-emerald-600/30">
              <p className="text-[9px] font-black uppercase tracking-widest mb-4 opacity-60">
                <Translate target={language}>Capital_Reclamation_Pulse</Translate>
              </p>
              <div className="flex items-baseline gap-2 mb-6">
                 ...
                 <span className="text-xs font-black opacity-50 uppercase">
                   <Translate target={language}>At_Risk_Assets</Translate>
                 </span>
              </div>
              <p className="text-xs font-bold italic leading-relaxed">
                "<Translate target={language}>Neural_Harvester_detects_92_evidence_coverage_for_current_denials_Projected_reclamation_window_14_business_days</Translate>"
              </p>
           </div>
        </div>
           ...
           {!selectedCase && !loading ? (
             <div className="flex-1 flex flex-col items-center justify-center text-center opacity-30">
                ...
                <h3 className="text-2xl font-black text-white uppercase tracking-widest">
                  <Translate target={language}>Awaiting_Case_Target</Translate>
                </h3>
                <p className="text-sm font-bold text-slate-500 mt-4 max-w-sm">
                  <Translate target={language}>The_Reclamation_Node_interrogates_the_Neural_Vault_to_harvest_evidence_refute_denials</Translate>
                </p>
             </div>
           ) : loading ? (
             <div className="flex-1 flex flex-col items-center justify-center space-y-10">
                <div className="w-24 h-24 border-4 border-emerald-500/10 border-t-emerald-500 rounded-full animate-spin"></div>
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.8em] animate-pulse">
                  <Translate target={language}>Harvesting_Clinical_Proof</Translate>
                </p>
             </div>
           ) : selectedCase && (
             <div className="flex-1 flex flex-col relative z-10 animate-in slide-in-from-bottom-8 duration-700">
                <div className="flex justify-between items-start mb-12">
                   <div>
                      <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
                        <Translate target={language}>Appeal_Vector</Translate>: {selectedCase.id}
                      </h3>
                      <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mt-4 italic">
                        <Translate target={language}>Denial_Code</Translate>: {selectedCase.denialCode}
                      </p>
                   </div>
                   <div className="text-right">
                      <p className="text-[8px] font-black text-slate-500 uppercase">
                        <Translate target={language}>Recovery_Possible</Translate>
                      </p>
                ...
                <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl mb-12">
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-6 italic">
                     <Translate target={language}>Grounded_Evidence_Harvested</Translate>
                   </p>
                      ...
                      {selectedCase.evidenceHarvested.map((e, i) => (
                        <div key={i} className="flex items-center gap-4 group">
                           ...
                           <p className="text-xs text-slate-200 font-medium italic">
                             "<Translate target={language}>{e}</Translate>"
                           </p>
                        </div>
                      ))}
                   </div>
                ...
                <div className="flex-1 flex flex-col min-h-0 bg-black/40 border border-white/5 rounded-3xl p-10 relative overflow-hidden">
                   ...
                   <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-8 italic">
                     <Translate target={language}>Drafted_Appeal_Brief</Translate>
                   </h4>
                ...
                <div className="mt-12 flex gap-4">
                   ...
                   >
                     <Translate target={language}>AUTHORIZE_&_TRANSMIT</Translate>
                   </button>
                   <button className="flex-1 py-6 bg-white/5 border border-white/10 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:text-white transition-all">
                     <Translate target={language}>Audit_Source_Logs</Translate>
                   </button>
                </div>
             </div>
           )}
