        ...
        <div className="flex bg-slate-900 p-1.5 rounded-2xl border border-white/10 shadow-xl">
          {['DEPLOYMENT', 'WAR_ROOM', 'BLASTS', 'COMPLAINTS'].map(v => (
            <button 
              ...
              className={`px-8 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${view === v ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-500 hover:text-white'}`}
            >
        ...
        {view === 'WAR_ROOM' ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
           ...
           <div className="bg-slate-900 border border-white/10 rounded-[4rem] p-12 shadow-2xl animate-in zoom-in duration-500">
              ...
              <div className="space-y-6 text-center opacity-30 italic py-32">No active shift blasts in the region.</div>
           </div>
        ) : (
          <div className="bg-rose-600/10 border border-rose-500/20 rounded-[4rem] p-12 shadow-2xl animate-in slide-in-from-right-8">
             ...
             <div className="py-32 text-center opacity-30 italic text-white">Consolidated complaint matrix is nominal.</div>
          </div>
        )}
      </div>
    </div>
