      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 bg-[#0a0f1e] border border-white/5 rounded-[4rem] p-16 shadow-2xl relative overflow-hidden flex flex-col min-h-[700px]">
           ...
           {loading ? (
             ...
             <div className="space-y-16 relative z-10 animate-in slide-in-from-bottom-8">
                <section>
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-6 italic">Institutional_Fragility_Points</p>
                      ...
                      {mandate.institutionalFragilityPoints.map((f, i) => (
                        <div key={i} className="p-8 bg-white/[0.02] border border-white/5 rounded-3xl group hover:bg-rose-500/5 transition-all">
                           <div className="flex items-center gap-4 mb-4">
                              ...
                              <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Failure Vector 0{i+1}</p>
                           </div>
                           <p className="text-sm text-slate-200 font-medium italic">"{f}"</p>
                        </div>
                      ))}
                   </div>
                </section>
                ...
                <section className="pt-10 border-t border-white/5">
                   <p className="text-[9px] font-black text-sky-400 uppercase tracking-widest mb-10">Market_Grounded_Outlook</p>
                   <div className="p-10 bg-sky-600/5 border border-sky-500/10 rounded-[3rem]">
                      <p className="text-lg text-slate-200 leading-relaxed font-medium italic italic">"{mandate.marketSentimentGrounded}"</p>
                   </div>
                </section>
             </div>
           )}
