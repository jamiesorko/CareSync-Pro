...
// Added useTranslate hook to provide translation logic for non-JSX contexts like input placeholders
export const useTranslate = (text: any, target?: string) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const cacheKey = `csp_v8_${language}_${source}`;
...
export const Translate: React.FC<{ children?: React.ReactNode; target?: string }> = ({ children, target }) => {
  ...
  return (
    <span key={`${language}-${source}`} className={`${loading ? 'opacity-40 animate-pulse' : 'transition-opacity duration-300'} inline`}>
      {translated}
    </span>
  );
};
